...
@HiltViewModel
class LatestMoviesViewModel @Inject constructor(private val apiRepository: ApiRepository) : BaseLifecycleViewModel<BaseViewState<LatestMoviesUiState>, LatestMoviesEvents>() {
    ...
    fun loadPopularMovies() {
        ...
        viewModelScope.launch {
            try {
                ...
                setState(BaseViewState.Data(LatestMoviesUiState(moviesList = _popularMovies)))
    ...
    override fun onTriggerEvent(eventType: LatestMoviesEvents) {
        when(eventType){
            is LatestMoviesEvents.LoadMovies -> loadPopularMovies()
            is LatestMoviesEvents.AddOrRemoveFromStored -> {}
            else -> {}
        }
    }
